...
import xxt.kareluo.imaging.core.XXTIMGImage;
import xxt.kareluo.imaging.core.XXTIMGMode;
import xxt.kareluo.imaging.core.XXTIMGPath;
import xxt.kareluo.imaging.core.XXTIMGText;
import xxt.kareluo.imaging.core.anim.XXTIMGHomingAnimator;
import xxt.kareluo.imaging.core.homing.XXTIMGHoming;
import xxt.kareluo.imaging.core.sticker.XXTIMGSticker;
import xxt.kareluo.imaging.core.sticker.XXTIMGStickerPortrait;

/**
 * Created by felix on 2017/11/14 下午6:43.
 */
// TODO clip外不加入path
public class XXTIMGView extends FrameLayout implements Runnable, ScaleGestureDetector.OnScaleGestureListener,
        ValueAnimator.AnimatorUpdateListener, XXTIMGStickerPortrait.Callback, Animator.AnimatorListener {

    private static final String TAG = "IMGView";

    private XXTIMGMode mPreMode = XXTIMGMode.NONE;

    private XXTIMGImage mImage = new XXTIMGImage();

    private GestureDetector mGDetector;

    private ScaleGestureDetector mSGDetector;

    private XXTIMGHomingAnimator mHomingAnimator;
...

    {
        ...
        mDoodlePaint.setStrokeWidth(XXTIMGPath.BASE_DOODLE_WIDTH);
        mDoodlePaint.setColor(Color.RED);
        mDoodlePaint.setPathEffect(new CornerPathEffect(XXTIMGPath.BASE_DOODLE_WIDTH));
        ...
        mMosaicPaint.setStrokeWidth(XXTIMGPath.BASE_MOSAIC_WIDTH);
        mMosaicPaint.setColor(Color.BLACK);
        mMosaicPaint.setPathEffect(new CornerPathEffect(XXTIMGPath.BASE_MOSAIC_WIDTH));
        mMosaicPaint.setStrokeCap(Paint.Cap.ROUND);
        mMosaicPaint.setStrokeJoin(Paint.Join.ROUND);
    }

    public XXTIMGView(Context context) {
        this(context, null, 0);
    }

    public XXTIMGView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XXTIMGView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void startHoming(XXTIMGHoming sHoming, XXTIMGHoming eHoming) {
        if (mHomingAnimator == null) {
            mHomingAnimator = new XXTIMGHomingAnimator();
    ...
    private void onDrawImages(Canvas canvas) {
        ...
        // 马赛克
        if (!mImage.isMosaicEmpty() || (mImage.getMode() == XXTIMGMode.MOSAIC && !mPen.isEmpty())) {
            int count = mImage.onDrawMosaicsPath(canvas);
            if (mImage.getMode() == XXTIMGMode.MOSAIC && !mPen.isEmpty()) {
                mDoodlePaint.setStrokeWidth(XXTIMGPath.BASE_MOSAIC_WIDTH);
                canvas.save();
        ...
        if (mImage.getMode() == XXTIMGMode.DOODLE && !mPen.isEmpty()) {
            mDoodlePaint.setColor(mPen.getColor());
            mDoodlePaint.setStrokeWidth(XXTIMGPath.BASE_DOODLE_WIDTH * mImage.getScale());
            canvas.save();
            RectF frame = mImage.getClipFrame();
            canvas.rotate(-mImage.getRotate(), frame.centerX(), frame.centerY());
            canvas.translate(getScrollX(), getScrollY());
        ...
        // 裁剪
        if (mImage.getMode() == XXTIMGMode.CLIP) {
            canvas.save();
            canvas.translate(getScrollX(), getScrollY());
    ...
    public <V extends View & XXTIMGSticker> void addStickerView(V stickerView, LayoutParams params) {
        if (stickerView != null) {
    ...
    public void addStickerText(XXTIMGText text) {
        XXTIMGStickerTextView textView = new XXTIMGStickerTextView(getContext());
    ...
    boolean onInterceptTouch(MotionEvent event) {
        if (isHoming()) {
            stopHoming();
            return true;
        } else if (mImage.getMode() == XXTIMGMode.CLIP) {
            return true;
        }
        return false;
    }
    ...
    boolean onTouch(MotionEvent event) {
        ...
        XXTIMGMode mode = mImage.getMode();

        if (mode == XXTIMGMode.NONE || mode == XXTIMGMode.CLIP) {
    ...
    public void onAnimationUpdate(ValueAnimator animation) {
        ...
        toApplyHoming((XXTIMGHoming) animation.getAnimatedValue());
    }


    private void toApplyHoming(XXTIMGHoming homing) {
    ...
    @Override
    public <V extends View & XXTIMGSticker> void onDismiss(V stickerView) {
        mImage.onDismiss(stickerView);
        invalidate();
    }

    @Override
    public <V extends View & XXTIMGSticker> void onShowing(V stickerView) {
        mImage.onShowing(stickerView);
        invalidate();
    }

    @Override
    public <V extends View & XXTIMGSticker> boolean onRemove(V stickerView) {
        if (mImage != null) {
    ...
    private boolean onScroll(float dx, float dy) {
        XXTIMGHoming homing = mImage.onScroll(getScrollX(), getScrollY(), -dx, -dy);
        if (homing != null) {
    ...
    private class MoveAdapter extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
            return XXTIMGView.this.onScroll(distanceX, distanceY);
        }
    ...
    private static class Pen extends XXTIMGPath {
        ...
        XXTIMGPath toPath() {
            return new XXTIMGPath(new Path(this.path), getMode(), getColor(), getWidth());
        }
    }
}
