...
import xxt.kareluo.imaging.widget.XXTIMGViewHolderCallback;

/**
 * Created by felix on 2018/1/5 下午1:51.
 */

public class XXTIMGGalleryMenuWindow extends PopupWindow {
    ...
    public XXTIMGGalleryMenuWindow(Context context) {
        super(context);
        mContext = context;

        View contentView = getLayoutInflater().inflate(
                R.layout.xxt_image_layout_gallery_pop, null, false);
        ...
        if (menuItems != null) {
            ...
            for (String item : menuItems) {
                GalleryMenuItemViewModel model = new GalleryMenuItemViewModel(
                        null, item, XXTIMGScanner.ALL_IMAGES.equals(item)
                );
                if (XXTIMGScanner.ALL_IMAGES.equals(item)) {
                    _all = model;
                }
    ...
    private class MenuAdapter extends RecyclerView.Adapter<MenuItemViewHolder>
            implements XXTIMGViewHolderCallback {

        @Override
        public MenuItemViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            return new MenuItemViewHolder(getLayoutInflater().inflate(
                    R.layout.xxt_image_layout_gallery_menu_item, parent, false), this);
        }
    ...
    private static class MenuItemViewHolder extends RecyclerView.ViewHolder
        ...
        private XXTIMGViewHolderCallback callback;

        public MenuItemViewHolder(View itemView, XXTIMGViewHolderCallback callback) {
            super(itemView);
